...
const state = {
  ...
  listPropStep: 1,          // List property: 1..3 алхам
  listPropDraft: null,      // { propertyType, district, khotkhon, khoroo, rooms, area, floor, year, desc, photos, price, deposit, contractMonths, negotiable, mortgage, features }
...
} catch (e) { /* private mode — алгасъя */ }
function saveMyPlaces() {
  try { localStorage.setItem(MY_PLACES_KEY, JSON.stringify(state.myPlaces || [])); } catch (e) {}
}
...
function makeInterestsProfileName(p) {
  const meta = (typeof INTEREST_LIFESTYLES !== 'undefined')
    ? INTEREST_LIFESTYLES.find(x => x.key === p.lifestyle) : null;
...
function syncActiveInterests() {
  ...
  if (!list.length) { state.userInterests = null; state.activeInterestsId = null; return; }
  const active = list.find(p => p.id === state.activeInterestsId);
  if (active) { state.userInterests = active; }
  else { state.activeInterestsId = list[0].id; state.userInterests = list[0]; }
}
...
try {
  ...
  if (raw) {
    ...
    if (parsed && Array.isArray(parsed.profiles)) {
      ...
      state.activeInterestsId = parsed.activeId || (state.userInterestsList[0] && state.userInterestsList[0].id) || null;
...
function saveInterestsDismissed() {
  try { localStorage.setItem(INTERESTS_DISMISS_KEY, JSON.stringify(state.interestsDismissedIds || [])); } catch (e) {}
}
...
function clearInterests() {
  ...
  try { localStorage.removeItem(INTERESTS_KEY); localStorage.removeItem(INTERESTS_DISMISS_KEY); } catch (e) {}
}
...
/* ============== ROUTER ============== */
const APP_SCREENS = ['home','results','property','schedule','confirmation','activity','saved','alerts','auth','profile','news','rental-mgmt','list-property','interests'];
...
function renderAppScreen(name) {
  ...
  if (name === 'home' && typeof initHomeLeafletMap === 'function') {
    setTimeout(() => {
      const pins = (state && state._homeMapPins) || (typeof modeListings === 'function' ? modeListings() : (window.LISTINGS || []));
...
function openLoanCompare() {
  ...
  if (!l) return;
  const downPct = state.loanDownPct, years = state.loanYears;
...
function pickRescheduleTime(t) {
  ...
  document.querySelectorAll('#reschedule-times [data-time]').forEach(b => b.classList.toggle('selected', b.dataset.time === t));
}
...
function buildSavedSearchDraft() {
  const districts = state.filterDistrict
    ...
    : (state.aiExtracted && state.aiExtracted.district ? [state.aiExtracted.district] : ['Хан-Уул', 'Сүхбаатар']);
    ...
    <div class="p-5 space-y-4">
      ...
      <div>
        ...
        <div class="grid grid-cols-3 gap-2">
          ${[['instant','Тэр даруй'],['daily','Өдөрт нэг'],['weekly','7 хоногт нэг']].map(([k,l], i) => `
...
function verifyOtp() {
  ...
  let code = '';
  inputs.forEach(i => { code += (i.value || '').replace(/[^\d]/g, ''); });
  ...
  openModal(`
    ...
    <div class="p-5">
      <div id="otp-boxes" class="flex justify-between gap-2 mb-2">
        ${[0,1,2,3,4,5].map(i => `<input class="otp-box" maxlength="1" inputmode="numeric" data-otp-i="${i}"
          oninput="handleOtpInput(this, ${i})" onkeydown="handleOtpKey(event, ${i})" />`).join('')}
      </div>
...
function submitSchedule() {
  ...
  const [mo, da] = String(state.scheduleDate || '').split('/').map(n => parseInt(n, 10));
  ...
  const dayLabel = daysUntil === 0 ? 'Өнөөдөр' : daysUntil === 1 ? 'Маргааш' : daysUntil === 2 ? 'Нөгөөдөр' : (daysUntil + ' хоног');
