...
function renderReviewsSection(listingId) {
  ...
  const all = [...userReviews.map(r => ({
    ...r,
    initials: r.initials || (r.author || '').split(/\s+/).map(w => w[0] || '').join('').replace(/\./g, '').slice(0, 2).toUpperCase(),
...
function startTour() {
  try { if (localStorage.getItem('neo_tour_seen') === '1') { window.__tourSeen = true; return; } } catch(e) {}
  tourState.step = 0;
...
function renderTour() {
  ...
  if (!el) {
    ...
    el.addEventListener('click', (ev) => { if (ev.target === el) endTour(); });
    document.body.appendChild(el);
  }
...
function endTour() {
  ...
  try { localStorage.setItem('neo_tour_seen', '1'); } catch(e) {}
}
...
function placeKindMeta(kind) { return PLACE_KINDS[kind] || PLACE_KINDS.other; }
...
function removePlace(id) {
  ...
  if (currentScreen === 'property') { renderAppScreen('property'); setTimeout(() => lucide.createIcons(), 0); }
}
...
function openPlacePicker(editId) {
  ...
  const defaultKind = !existingKinds.has('home') ? 'home'
                    : !existingKinds.has('work') ? 'work'
                    : !existingKinds.has('school') ? 'school'
                    : !existingKinds.has('kindergarten') ? 'kindergarten'
                    : 'work';
...
function renderPlacePickerModal() {
  ...
  const kindBtns = Object.entries(PLACE_KINDS).map(([k, m]) => {
    const sel = pp.kind === k;
  ...
  const sameKindCount = placesByKindCount(pp.kind) - (pp.editId && pp.kind === (existingPlaces.find(p=>p.id===pp.editId)||{}).kind ? 1 : 0);
  const labelRequired = sameKindCount > 0;
  const labelPlaceholder = pp.kind==='home' ? (sameKindCount>0?'ж: Аав ээжийнх':'ж: Манай гэр')
    : pp.kind==='work' ? (sameKindCount>0?'ж: Эхнэрийн ажил':'ж: Миний оффис')
    : pp.kind==='school' ? (sameKindCount>0?'ж: Дүүгийн сургууль':'ж: Хүүгийн сургууль')
    : pp.kind==='kindergarten' ? (sameKindCount>0?'ж: Дунд хүүгийн цэцэрлэг':'ж: Том хүүгийн цэцэрлэг')
    : 'ж: Эмнэлэг, эцэг эх...';
...
window.__refreshPickerSaveBtn = function() {
  ...
  if (hasPoint && labelOK) { btn.style.opacity = '1'; btn.style.pointerEvents = 'auto'; }
...
function destroyPlacePickerLeafletMap() {
  if (placePickerMap) {
    try { placePickerMap.remove(); } catch (e) {}
  }
  placePickerMap = null;
...
function initPlacePickerLeafletMap() {
  ...
  function setMarkerAt(realLat, realLng) {
    ...
    setTimeout(() => { try { if (window.lucide) lucide.createIcons(); } catch(e){} }, 0);
  ...
  setTimeout(() => { try { if (placePickerMap) placePickerMap.invalidateSize(); } catch (e) {} }, 50);
}
...
function useMyLocationForPicker() {
  ...
  navigator.geolocation.getCurrentPosition(
    pos => {
      ...
      if (latitude < UB_BOUNDS.latMin - 0.5 || latitude > UB_BOUNDS.latMax + 0.5 ||
          longitude < UB_BOUNDS.lngMin - 0.5 || longitude > UB_BOUNDS.lngMax + 0.5) {
        showToast('Та одоо УБ-аас гадна байгаа бололтой. Газрын зураг дээр гараар сонгоно уу.', 'warning', { duration: 3000 });
        return;
      }
...
function shouldShowOnboarding() {
  ...
  if ((state.myPlaces || []).length > 0) return false;
  try {
  ...
  try { if (localStorage.getItem(ONBOARD_DISMISSED_KEY)) return false; } catch (e) {}
  return true;
}
...
function maybeShowOnboarding() {
  ...
  setTimeout(() => { if (shouldShowOnboarding()) renderOnboardingModal(); }, 400);
}
...
function renderOnboardingModal() {
  ...
  root.addEventListener('click', (e) => { if (e.target === root) onboardSkip(); });
  root.innerHTML = `
    <div class="onboard-card" onclick="event.stopPropagation()">
      ...
      <div class="onboard-body">
        ...
        <button class="onboard-kind-row" onclick="onboardStart('work')">
          <div class="onboard-kind-icon" style="background:${PLACE_KINDS.work.color};">
            <i data-lucide="briefcase" style="width:20px;height:20px;"></i>
          </div>
          <div class="onboard-kind-text">
            <div class="onboard-kind-title">Ажлын байршил</div>
            <div class="onboard-kind-desc">Гэрээс ажил хүртэлх зайг тооцоолно</div>
          </div>
          <i data-lucide="chevron-right" class="onboard-kind-arrow" style="width:16px;height:16px;"></i>
        </button>

        <button class="onboard-kind-row" onclick="onboardStart('school')">
          <div class="onboard-kind-icon" style="background:${PLACE_KINDS.school.color};">
            <i data-lucide="graduation-cap" style="width:20px;height:20px;"></i>
          </div>
          <div class="onboard-kind-text">
            <div class="onboard-kind-title">Хүүхдийн сургууль</div>
            <div class="onboard-kind-desc">Олон сургууль нэмэх боломжтой</div>
          </div>
          <i data-lucide="chevron-right" class="onboard-kind-arrow" style="width:16px;height:16px;"></i>
        </button>

        <button class="onboard-kind-row" onclick="onboardStart('kindergarten')">
          <div class="onboard-kind-icon" style="background:${PLACE_KINDS.kindergarten.color};">
            <i data-lucide="baby" style="width:20px;height:20px;"></i>
          </div>
          <div class="onboard-kind-text">
            <div class="onboard-kind-title">Цэцэрлэг</div>
            <div class="onboard-kind-desc">Бага насны хүүхдийн цэцэрлэгийн байршил</div>
          </div>
          <i data-lucide="chevron-right" class="onboard-kind-arrow" style="width:16px;height:16px;"></i>
        </button>

        <button class="onboard-kind-row" onclick="onboardStart('home')">
          <div class="onboard-kind-icon" style="background:${PLACE_KINDS.home.color};">
            <i data-lucide="home" style="width:20px;height:20px;"></i>
          </div>
          <div class="onboard-kind-text">
            <div class="onboard-kind-title">Одоогийн оршин суух газар</div>
            <div class="onboard-kind-desc">Шилжих санал өгөхөд хэрэг болно</div>
          </div>
          <i data-lucide="chevron-right" class="onboard-kind-arrow" style="width:16px;height:16px;"></i>
        </button>
      </div>
...
function confirmRemovePlace(id) {
  if (confirm('Энэ газрыг устгах уу?')) {
    removePlace(id);
    closePlacePicker();
    if (currentScreen === 'home') { renderAppScreen('home'); }
  }
}
...
function savePlacePicker() {
  ...
  if (currentScreen === 'home') { renderAppScreen('home'); setTimeout(() => lucide.createIcons(), 0); }
  else if (currentScreen === 'property') { renderAppScreen('property'); setTimeout(() => lucide.createIcons(), 0); }
}
...
const UB_BOUNDS = { latMin: 47.875, latMax: 47.965, lngMin: 106.820, lngMax: 107.040 };
...
function declutterHomeLeafletMarkers(items, el) {
  ...
  const markerH = 44;
  const pad = 8;
  const blockers = [...document.querySelectorAll('.home-ai-panel:not(.collapsed), .home-stats-strip, .home-topbar, .qf-bar')]
  ...
  sorted.forEach(item => {
    ...
    for (const [dx, dy] of offsets) {
      ...
      const inMap = rect.left >= mapRect.left + pad
...
function destroyHomeLeafletMap() {
  if (homeLeafletMap) {
    try { homeLeafletMap.remove(); } catch (e) {}
    homeLeafletMap = null;
    homeLeafletMarkers = [];
  }
}
...
function initHomeLeafletMap(listings) {
  ...
  homeLeafletMap = L.map(el, {
    ...
    minZoom: 11,
    maxZoom: 18,
    maxBounds: [[UB_BOUNDS.latMin - 0.1, UB_BOUNDS.lngMin - 0.1], [UB_BOUNDS.latMax + 0.1, UB_BOUNDS.lngMax + 0.1]],
  ...
  (listings || []).forEach(l => {
    ...
    setTimeout(() => {
      ...
      if (el) { el.dataset.listingId = String(l.id); }
    }, 0);
    ...
    marker.on('tooltipopen', () => { setTimeout(() => { try { lucide.createIcons(); } catch(e){} }, 0); });
    marker.on('click', () => openProperty(l.id));
    marker.on('mouseover', () => { if (window.highlightHomeCard) window.highlightHomeCard(l.id, true); });
    marker.on('mouseout',  () => { if (window.highlightHomeCard) window.highlightHomeCard(null, false); });
  ...
  if (listingBounds.length === 1) {
    ...
    const rightPadding = aiWidth
      ? Math.min(Math.round(aiWidth + 260), Math.round(el.clientWidth * 0.58))
      : 130;
